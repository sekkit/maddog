...
func TestSaveCachedUpdateMarksEvaluateDownloaded(t *testing.T) {
	...
	asset := update.Asset{
		URL:    "https://dl.maddog.io/desktop-v9.9.9/Maddog-linux-amd64.tar.gz",
		Size:   int64(len(data)),
...
func TestExtractBinary(t *testing.T) {
	...
	tw := tar.NewWriter(gz)
	files := map[string][]byte{"README": []byte("ignore me"), "maddog": want}
	...
	got, err := extractBinary(buf.Bytes(), "maddog")
	if err != nil {
