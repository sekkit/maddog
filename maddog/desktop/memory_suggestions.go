...
func workflowCategories() []workflowCategory {
	return []workflowCategory{
		{
			Name:        "maddog-pr-followup",
			Description: "Review or update a Maddog GitHub PR, address feedback, verify, and publish safely.",
		...
		{
			Name:        "maddog-memory-ui",
			Description: "Iterate on the Maddog desktop Memory page with source-backed UI decisions and browser verification.",
...
func skillCandidateBody(cat workflowCategory, evidence []string) string {
	var b strings.Builder
	title := strings.TrimPrefix(strings.ReplaceAll(cat.Name, "-", " "), "maddog ")
	b.WriteString("# " + cases.Title(language.Und).String(title) + "\n\n")
	b.WriteString("Use this skill when the user asks for this repeated Maddog workflow.\n\n")
