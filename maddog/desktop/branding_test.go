...
func TestWailsDevBinaryNameIsMaddog(t *testing.T) {
	...
	if !strings.Contains(strings.ToLower(cfg.Name), "maddog") {
		t.Fatalf("wails app name should contain maddog: %q", cfg.Name)
	}
}

func TestDesktopIdentityUsesMaddogNamespace(t *testing.T) {
	if singleInstanceID != "com.maddog.desktop" {
		t.Fatalf("singleInstanceID = %q, want com.maddog.desktop", singleInstanceID)
	}
}
