...
// (maddog.key) and the public key (maddog.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "maddog.key")
	pubPath := filepath.Join(dir, "maddog.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "Maddog desktop release")
		out := f + ".minisig"
