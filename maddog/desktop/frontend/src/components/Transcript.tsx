...
import { ArrowDown, ChevronRight, GitPullRequestArrow, MessagesSquare, Route, ShieldAlert, Sparkles } from "lucide-react";
...
function RuntimeEventCard({ event, level, text }: { event: RuntimeEventItem["event"]; level: RuntimeEventItem["level"]; text: string }) {
  const t = useT();
  const spec = runtimeEventSpec(event, level, t);
  return (
    <ProcessCard
      tone={spec.tone}
      icon={spec.icon}
      kind={spec.kind}
      name={spec.title}
      meta={<ProcessStatusIcon state={spec.state} label={spec.title} />}
      defaultOpen
...
function runtimeEventSpec(event: RuntimeEventItem["event"], level: RuntimeEventItem["level"], t: ReturnType<typeof useT>) {
  if (event === "upgrade") {
    return { tone: "accent" as const, icon: <Route size={12} />, kind: t("runtimeEvent.route"), title: t("runtimeEvent.upgrade"), state: "done" as const };
  }
  if (event === "budget_exceeded") {
    return { tone: "warning" as const, icon: <ShieldAlert size={12} />, kind: t("runtimeEvent.route"), title: t("runtimeEvent.budget"), state: "waiting" as const };
  }
  if (event === "skill_promoted") {
    return { tone: "success" as const, icon: <GitPullRequestArrow size={12} />, kind: t("runtimeEvent.skill"), title: t("runtimeEvent.promoted"), state: "done" as const };
  }
  return { tone: level === "warn" ? "warning" as const : "violet" as const, icon: <Sparkles size={12} />, kind: t("runtimeEvent.skill"), title: t("runtimeEvent.generated"), state: "done" as const };
}
