...
const TRACE_STRING_LIMIT = 8192;
const TRACE_ARRAY_LIMIT = 100;
...
function redactValue(value: unknown, key = "", depth = 0): unknown {
  if (depth > TRACE_MAX_DEPTH) return "[truncated: max depth]";
  if (typeof value === "string") {
    if (SECRET_KEY_RE.test(key)) return "[redacted]";
    let sanitized = redactSerializedJSON(value, depth);
    sanitized = sanitized.replace(SECRET_VALUE_RE, "[redacted]");
    if (sanitized.length > TRACE_STRING_LIMIT) {
      return `${sanitized.slice(0, TRACE_STRING_LIMIT)}… [truncated ${sanitized.length - TRACE_STRING_LIMIT} chars]`;
    }
    return sanitized;
  }
  if (Array.isArray(value)) {
    const out = value.slice(0, TRACE_ARRAY_LIMIT).map((item) => redactValue(item, key, depth + 1));
    if (value.length > TRACE_ARRAY_LIMIT) out.push(`[truncated ${value.length - TRACE_ARRAY_LIMIT} items]`);
    return out;
  }
  if (value && typeof value === "object") {
    const out: Record<string, unknown> = {};
    for (const [childKey, childValue] of Object.entries(value)) {
      out[childKey] = SECRET_KEY_RE.test(childKey) ? "[redacted]" : redactValue(childValue, childKey, depth + 1);
    }
    return out;
  }
  return value;
...
function redactSerializedJSON(value: string, depth: number): string {
  const trimmed = value.trim();
  if ((!trimmed.startsWith("{") && !trimmed.startsWith("[")) || value.length > 262144) return value;
  try {
    return JSON.stringify(redactValue(JSON.parse(value), "", depth + 1));
  } catch {
    return value;
  }
}
