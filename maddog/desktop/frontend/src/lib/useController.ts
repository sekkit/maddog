...
function applyEvent(s: State, e: WireEvent): State {
  ...
  switch (e.kind) {
    ...
    case "tool_result": {
      ...
      if (idx >= 0) {
        ...
        if (it.kind === "tool") {
          ...
          const existing = it;
          const shortArgs = archivedToolArgs(existing.name, existing.args);
    ...
    case "skill_promoted": {
      const text = e.text ?? "";
      const routeStatus = e.kind === "upgrade" || e.kind === "budget_exceeded" ? text : s.routeStatus;
      const skillStatus = e.kind === "skill_generated" || e.kind === "skill_promoted" ? text : s.skillStatus;
      return {
        ...s,
