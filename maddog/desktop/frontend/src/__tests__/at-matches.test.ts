...
{
  const entries: DirEntry[] = [];
  const searchEntries = [externalEntry("__maddog_external_folder/abc/Folder/src/outside.txt", "Folder With Spaces/src/outside.txt")];
  const got = filterAtMatches(entries, searchEntries, "spaces");
  eq(
    got.map((e) => e.path),
    ["__maddog_external_folder/abc/Folder/src/outside.txt"],
...
{
  const entries = [externalEntry("__maddog_external_folder/abc/Folder/src/outside.txt", "outside.txt")];
  const searchEntries = [externalEntry("__maddog_external_folder/abc/Folder/src/outside.txt", "Folder With Spaces/src/outside.txt")];
