...
const fastContext = before.codeBackends.find((backend) => backend.id === "fastcontext-style");
ok("capabilities exposes FastContext-style backend by default", Boolean(
  fastContext?.enabled && fastContext.health === "available" && fastContext.capabilities.includes("semantic_search"),
));
...
const fastSummary: CodeBackendBenchmarkView = await app.RunCodeBackendBenchmark("fastcontext-style");
ok("FastContext-style benchmark binding returns summary", fastSummary.backendId === "fastcontext-style" && fastSummary.path.length > 0);
