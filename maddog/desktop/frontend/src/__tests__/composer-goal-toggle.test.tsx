...

{
  const dom = installDom();
  mockApp({
    SavePastedFile: async () => ".maddog/attachments/notes.txt",
  });
...

{
  ...
  mockApp({
    AttachDropped: async () => ({
      kind: "attachment",
      path: ".maddog/attachments/report.pdf",
    }),
  });
  const { root } = await renderComposer();
  if (!droppedCallback) throw new Error("native file drop handler did not register");
...

{
  ...
  mockApp({
    AttachDropped: async () => ({
      kind: "workspace",
      path: "__maddog_external_folder/mock/Folder-With-Spaces",
      isDir: true,
  ...
  eq(calls.submit.join(","), "inspect @__maddog_external_folder/mock/Folder-With-Spaces/", "external folder submit text uses the session ref token");
...
{
  const externalToken = "__maddog_external_folder/mock/Folder-With-Spaces/src/outside.txt";
