...

{
  ...
  const meta: Meta = { label: "test", ready: true, eventChannel: "test-channel", cwd: "/tmp/maddog-test", workspaceRoot: "/tmp/maddog-test" };
  const tabs: TabMeta[] = [{
    id: "tab-1",
    scope: "project",
    workspaceRoot: "/tmp/maddog-test",
    workspaceName: "maddog-test",
    topicId: "topic-1",
    topicTitle: "Test",
    label: "Test",
    ready: true,
...

{
  ...
  await waitFor("legacy binding fallback server row", () => Boolean(document.querySelector(".cap-row__name")?.textContent?.includes("legacy-binding")));
