...
effort = "max"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "maddog.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
...
`
	if err := os.WriteFile(filepath.Join(root, "maddog.toml"), []byte(configBody), 0o644); err != nil {
		t.Fatal(err)
	}
