...
`desktop/` is its own Go module (`module maddog/desktop`, `replace maddog =>
...
`maddog/` so it can still import the `maddog/internal/*` kernel.
...
- **macOS** — open `Maddog-darwin-universal.dmg` and drag Maddog into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/Maddog.app
  ```
...
```sh
minisign -Vm Maddog-darwin-arm64.zip \
...
- **Linux / WebKitGTK** is the one real pain point — rendering varies by distro &
  ...
  - **Wayland + NVIDIA**: On KDE Plasma Wayland with NVIDIA GPUs, WebKitGTK can
    ...
    Maddog automatically sets `__NV_DISABLE_EXPLICIT_SYNC=1` when it detects
...
- **Windows / WebView2** — `Theme: SystemDefault` follows the OS light/dark
  ...
  `MADDOG_DESKTOP_DISABLE_WEBVIEW2_GPU=1` or `0` to force the fallback on or
  off.
...
The desktop app sends one anonymous ping per launch to `crash.maddog.io`:
...
`agent.memory_compiler.enabled` setting with the CLI/TUI and `maddog serve`;
CLI users can also run `/memory-v5 off|on|status` in a session or
`maddog config memory-v5 off|on|status` from a shell.
