...
func isolateDesktopUserDirs(t *testing.T) string {
	...
	t.Setenv("MADDOG_CACHE_HOME", filepath.Join(home, "cache"))
...
func TestNeedsOnboardingIgnoresInheritedEnv(t *testing.T) {
	...
	if !app.NeedsOnboarding() {
		t.Fatal("NeedsOnboarding should require a key saved in Maddog global .env")
	}
...
`), 0o644); err != nil {
	...
	got := NewApp().Settings()
	if got.DesktopLanguage != "en" || got.DesktopLayoutStyle != "workbench" || got.DesktopTheme != "dark" || got.DesktopThemeStyle != "graphite" || got.CloseBehavior != "background" || got.StatusBarStyle != "text" {
...
api_key_env = "DEEPSEEK_API_KEY"
`
	if err := os.WriteFile(filepath.Join(projectRoot, "maddog.toml"), []byte(projectConfig), 0o644); err != nil {
		t.Fatalf("write project config: %v", err)
	}
...
func TestDeleteProviderRejectsUnaffectedBackgroundJobsBeforeSavingConfig(t *testing.T) {
	isolateDesktopUserDirs(t)
	setDesktopTestCredential(t, "MADDOG_TEST_KEY", "sk-test")
...
func TestCapabilitiesShowsDefaultCodegraphEnabled(t *testing.T) {
	isolateDesktopUserDirs(t)
	dir := robustTempDir(t)
	t.Chdir(dir)
	...
	for _, s := range view.Servers {
		if s.Name == "codegraph" {
			if s.Status != "initializing" {
				t.Fatalf("codegraph status = %q, want initializing; server = %+v", s.Status, s)
			}
			if !s.BuiltIn || !s.Configured {
				t.Fatalf("codegraph builtIn/configured = %v/%v, want true/true; server = %+v", s.BuiltIn, s.Configured, s)
			}
			if !s.AutoStart {
				t.Fatalf("codegraph autoStart = false, want true; server = %+v", s)
			}
...
func TestSetMCPServerEnabledSharedHostPreservesSiblingTabs(t *testing.T) {
	...
	defer srv.Close()
	if err := os.WriteFile(filepath.Join(dir, "maddog.toml"), []byte(fmt.Sprintf(`
[[plugins]]
name = "h"
type = "http"
url = %q
...
func TestUpdateMCPServerEditsProjectMCPJSONEntry(t *testing.T) {
  ...
  "mcpServers": {
	...
	if got.Command != "maddog-missing-mcp-binary" || !reflect.DeepEqual(got.Args, []string{"serve", "--mcp"}) || got.Env["CODEGRAPH_LOG"] != "debug" {
...
func TestBuiltInMCPBackgroundNotifyDoesNotDownload(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv("MADDOG_CACHE_DIR", robustTempDir(t))
