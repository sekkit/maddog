...
const macBundleID = "com.wails.maddog-desktop"
...
func applyMac(zipPath string) error {
	...
	staging, err := os.MkdirTemp("", "maddog-mac-update-*")
	if err != nil {
		return err
	}
	handedOff := false
	...
	script := filepath.Join(staging, "install-maddog-update.sh")
	body := fmt.Sprintf(`#!/bin/sh
set -eu
old_app=%q
new_app=%q
backup_app="$old_app.maddog-update-backup"
sleep 1
rm -rf "$backup_app"
...
func findMacApp(root string) (string, error) {
	direct := filepath.Join(root, "Maddog.app")
