...
func TestRegistryFromConfigRegistersBuiltInDefaultBackend(t *testing.T) {
	...
	if len(backends) != 2 {
		t.Fatalf("Backends len = %d, want built-in and FastContext-style: %+v", len(backends), backends)
	}
	b := backends[0]
	...
	fast, ok := reg.Backend(FastContextStyleBackendID)
	if !ok {
		t.Fatalf("FastContext-style backend missing: %+v", backends)
	}
	if fast.Default || !fast.BuiltIn || !fast.Enabled || fast.Health != BackendHealthAvailable {
		t.Fatalf("FastContext-style backend = %+v, want enabled non-default built-in", fast)
	}
	if !hasBackendCapability(fast.Capabilities, BackendCapabilitySemanticSearch) {
		t.Fatalf("FastContext-style capabilities = %+v, want semantic search", fast.Capabilities)
	}
}
