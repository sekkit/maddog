...
func TestMain(m *testing.M) {
	dir, err := os.MkdirTemp("", "maddog-installsource-test-*")
	if err != nil {
	...
	_ = os.Setenv("MADDOG_HOME", filepath.Join(dir, ".maddog"))
	_ = os.Setenv("MADDOG_STATE_HOME", filepath.Join(dir, ".maddog-state"))
	_ = os.Setenv("MADDOG_CACHE_HOME", filepath.Join(dir, ".maddog-cache"))
	_ = os.Setenv("MADDOG_CREDENTIALS_STORE", "file")
	_ = os.Setenv("XDG_CONFIG_HOME", filepath.Join(dir, ".config"))
	_ = os.Setenv("AppData", filepath.Join(dir, "AppData"))
	_ = os.Setenv("APPDATA", filepath.Join(dir, "AppData"))
	_ = os.Setenv("LOCALAPPDATA", filepath.Join(dir, "AppData"))
	code := m.Run()
	_ = os.RemoveAll(dir)
	os.Exit(code)
}
...
func TestApplyLocalSkillRootRegistersPath(t *testing.T) {
	...
	cfg := config.LoadForEdit(filepath.Join(project, "maddog.toml"))
...
func TestApplyLocalSkillFileCopiesToProject(t *testing.T) {
	...
	target := filepath.Join(project, ".maddog", "skills", "beta", "SKILL.md")
...
func TestApplyLocalSkillFileDoesNotShadowFlatCompatInstall(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	existing := filepath.Join(project, ".maddog", "skills", "beta.md")
...
func TestApplyLocalSKILLFileCopiesSiblingResources(t *testing.T) {
	...
	target := filepath.Join(project, ".maddog", "skills", "frontend-design", "SKILL.md")
	if resp.Actions[0].CanonicalPath != target {
		t.Fatalf("canonicalPath = %q, want %q", resp.Actions[0].CanonicalPath, target)
	}
	if _, err := os.Stat(filepath.Join(project, ".maddog", "skills", "frontend-design", "references", "style.md")); err != nil {
		t.Fatalf("reference file should be copied with SKILL.md source: %v", err)
	}
	if _, err := os.Stat(filepath.Join(project, ".maddog", "skills", "frontend-design", "scripts", "lint.sh")); err != nil {
...
func TestApplyLocalSkillLinkMode(t *testing.T) {
	...
	target := filepath.Join(project, ".maddog", "skills", "gamma", "SKILL.md")
...
func TestPlanProjectMCPJSONDefaultsProject(t *testing.T) {
  ...
  "mcpServers": {
	...
	wantPath := filepath.Join(project, "maddog.toml")
...
func TestApplyRemoteMCPURLDefaultsGlobal(t *testing.T) {
	...
	projectCfg := config.LoadForEdit(filepath.Join(project, "maddog.toml"))
...
func TestApplyMCPRejectsDuplicateByDefault(t *testing.T) {
	...
	if err := cfg.SaveTo(filepath.Join(project, "maddog.toml")); err != nil {
		t.Fatal(err)
	}
...
func TestApplyMCPReplaceOverwritesExisting(t *testing.T) {
	...
	reloaded := config.LoadForEdit(filepath.Join(project, "maddog.toml"))
...
func TestApplyMCPRollsBackOnSaveFailure(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(project, "maddog.toml"), 0o755); err != nil {
		t.Fatal(err)
	}
	writeFile(t, filepath.Join(project, "maddog.toml", "blocker"), "x")
...
func TestGlobalSkillInstallRootUsesMaddogHome(t *testing.T) {
	home := t.TempDir()
	...
	want := filepath.Join(maddogHome, skill.SkillsDirname)
	if root != want {
...
func TestUninstallRemovesSkillByName(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	target := filepath.Join(project, ".maddog", "skills", "doomed.md")
	writeFile(t, target, "---\nname: doomed\ndescription: Doomed\n---\nbody")
...
func TestUninstallWithoutScopePrefersProjectSkill(t *testing.T) {
	project := t.TempDir()
	home := t.TempDir()
	projectTarget := filepath.Join(project, ".maddog", "skills", "dupe.md")
	globalTarget := filepath.Join(home, ".maddog", "skills", "dupe.md")
...
func TestPlanIDIncludesActionDetails(t *testing.T) {
	...
	a := action{Kind: "mcp", Action: "install_mcp_server", Name: "same", URL: "https://mcp.one.example/mcp", Transport: "http", ConfigPath: "/repo/maddog.toml"}
	b := a
