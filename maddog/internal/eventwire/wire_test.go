...
func TestToWireMessageMemoryCitations(t *testing.T) {
	...
	if got.Source != "MEMORY.md" || got.LineStart != 116 || got.LineEnd != 123 || got.Note != "maddog workflow" {
...
func TestToWireInteractionAndLifecyclePayloads(t *testing.T) {
	tests := []struct {
		...
		{
			name: "runtime event",
			in:   event.Event{Kind: event.BudgetExceeded, Text: "budget reached", Level: event.LevelWarn},
			want: []string{`"kind":"budget_exceeded"`, `"text":"budget reached"`, `"level":"warn"`},
		},
		{
			name: "advisor",
			in: event.Event{Kind: event.Advisor, Text: "advisor ready", Advisor: event.AdvisorConsultation{
				Reason: "risk", Question: "Proceed?", Advice: "Add tests",
				UsesThisTurn: 1, UsesThisSession: 2, RemainingThisTurn: 3,
				RemainingThisSession: 4, MaxUsesPerTurn: 5, MaxUsesPerSession: 6,
			}},
			want: []string{
				`"kind":"advisor"`, `"level":"info"`, `"text":"advisor ready"`, `"advisor":{`,
				`"reason":"risk"`, `"question":"Proceed?"`, `"advice":"Add tests"`,
				`"usesThisTurn":1`, `"remainingThisSession":4`, `"maxUsesPerSession":6`,
			},
		},
	}
