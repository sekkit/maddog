...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "maddog-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "maddog-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "maddog-missing-mcp-binary" not found on PATH`,
