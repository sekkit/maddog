...
func TestMCPMigrationWaitsForCLIWorkspace(t *testing.T) {
	isolateCLIConfigHome(t)
	cwd := mustGetwd(t)
	if err := os.WriteFile(filepath.Join(cwd, "maddog.toml"), []byte(`
[[plugins]]
name = "cwd-project"
...
func TestRunDoesNotMigrateMaddogConfigBeforeConfigOnlyCommands(t *testing.T) {
	isolateCLIConfigHome(t)
	legacyPath := filepath.Join(filepath.Dir(config.UserConfigPath()), "maddog.toml")
...
`), 0o644); err != nil {
	...
	if strings.Contains(out, "legacy-cli") {
		t.Fatalf("mcp list should not include original Maddog config:\n%s", out)
	}
	if _, err := os.Stat(config.UserConfigPath()); !os.IsNotExist(err) {
		t.Fatalf("config command should not migrate original Maddog config, stat err=%v", err)
	}
}
...
func TestConfigMemoryV5LocalIsRejected(t *testing.T) {
	...
	if _, err := os.Stat("maddog.toml"); !os.IsNotExist(err) {
		t.Fatalf("maddog.toml should not be written, stat err=%v", err)
	}
}
...
func TestConfigAutoPlanIgnoresProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile("maddog.toml", []byte("[agent]\nauto_plan = \"on\"\n"), 0o644); err != nil {
...
func TestFetchOrFallback(t *testing.T) {
	...
	t.Run("no key set returns static list (offline first-run)", func(t *testing.T) {
		t.Setenv("MADDOG_FETCH_TEST_KEY", "")
		probe := config.ProviderEntry{
			BaseURL:   "http://127.0.0.1:1", // unreachable, no listener
			APIKeyEnv: "MADDOG_FETCH_TEST_KEY",
...
// with a single model). Re-running `maddog setup` must still surface the
...
func TestWriteDefaultConfigEnablesCodegraph(t *testing.T) {
	...
	if c := config.LoadForEdit(path); !c.Codegraph.Enabled {
		t.Fatal("a freshly scaffolded config disabled codegraph; v1 defaults should enable it")
	}
}
