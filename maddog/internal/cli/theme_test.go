...
func TestConfigureCLIThemeHonorsMaddogEnvOverride(t *testing.T) {
	t.Setenv("COLORTERM", "")
	t.Setenv("TERM_PROGRAM", "")
	t.Setenv("MADDOG_THEME", "")
	t.Setenv("MADDOG_THEME_STYLE", "")
	t.Setenv("MADDOG_THEME", "ember")
	t.Setenv("MADDOG_THEME_STYLE", "aurora")
	defer restoreThemeForTest(colorEnabled, activeCLITheme)
	colorEnabled = true

	configureCLIThemeWithStyle("light", "glacier")
	if activeCLITheme.name != "dark" || activeCLITheme.style != "aurora" {
		t.Fatalf("MADDOG_THEME override resolved %s/%s, want dark/aurora", activeCLITheme.name, activeCLITheme.style)
	}
}
