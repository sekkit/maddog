...
type chatTUI struct {
	...
	// left untouched.
	planMode bool
	// sessionSwitch is set by replayActiveBranch/resetFreshContextView to suppress
	// ClearScreen flicker when viewport content is rebuilt during a session switch.
	sessionSwitch bool
	...
	// commands are custom slash commands loaded from .maddog/commands and legacy dirs; each renders
...
func renderTUIBanner(label, missing string, width int) string {
	var b strings.Builder
	b.WriteString(accent("◆") + " " + bold("maddog chat") + "  " + dim("· "+label) + "\n")
...
var cliImageRefRe = regexp.MustCompile(`(?:^|\s)@\.maddog/attachments/clipboard-\d{8}-\d{6}\.\d+(?:-(?:\d{6}|[a-f0-9]{8}))?\.(?:png|jpg|jpeg|gif|webp)`)
