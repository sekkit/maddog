...
func TestMain(m *testing.M) {
	...
	// the environment (MADDOG_LANG/LC_ALL/LC_MESSAGES/LANG) and installs it as
...
func writeTUIImageCapabilityConfig(t *testing.T, root string) {
	...
	if err := cfg.SaveTo(filepath.Join(root, "maddog.toml")); err != nil {
...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"maddog chat", "› /mcp"}
	...
	if !strings.Contains(lines[0], "maddog chat") || !strings.Contains(lines[1], "/mcp") {
...
func TestIngestEventRoutesByKind(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"usage-pxpipe", event.Event{Kind: event.Usage, Usage: &provider.Usage{PromptTokens: 1000, CompletionTokens: 200, TotalTokens: 1200}, Pxpipe: &event.PxpipeSummary{Requests: 2, Compressed: 1, PassThrough: 1, Images: 3}}, "pxpipe 1 compressed / 1 pass-through, 3 images"},
...
func TestAutoPlanCommandWritesUserConfigNotProjectConfig(t *testing.T) {
	isolateUserConfig(t)
	projectPath := filepath.Join(mustGetwd(t), "maddog.toml")
...
func TestLanguageCommandAutoClearsLowerPriorityUserOverride(t *testing.T) {
	...
	if err := projectCfg.SaveTo("maddog.toml"); err != nil {
...
func TestSlashMigrateFromImportsExplicitSessions(t *testing.T) {
	...
	legacySessions := filepath.Join(home, "Old Maddog", "sessions")
