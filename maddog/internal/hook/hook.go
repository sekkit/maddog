...
// (.maddog/settings.json, only when the project is trusted) and a global
// (~/.maddog/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = config.ProjectConventionDir
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.maddog/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(maddogHome(homeDir), SettingsFilename)
}

// ProjectSettingsPath is <root>/.maddog/settings.json.
...
func maddogHome(override string) string {
	...
	if dir := config.MaddogHomeDir(); dir != "" {
		return dir
	}
...
func legacyMaddogHome(override string) string {
	if override != "" {
		return ""
	}
	...
	if sameCleanPath(legacy, maddogHome("")) {
		return ""
	}
	return legacy
}
