...
func TestPluginMutators(t *testing.T) {
	...
	if err := c.UpsertPlugin(PluginEntry{Name: "ex", Command: "maddog-plugin-example"}); err != nil {
...
func TestLoadForEditPreservesCodegraphWithoutSection(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "maddog.toml")
...
func TestLoadFirstRunEnablesCodegraph(t *testing.T) {
	...
	if !cfg.Codegraph.Enabled {
		t.Fatal("first run (no config file anywhere) disabled codegraph; v1 code intelligence should start enabled")
	}
}
...
func TestSaveToRoundTrips(t *testing.T) {
	...
	path := filepath.Join(t.TempDir(), "nested", "maddog.toml")
