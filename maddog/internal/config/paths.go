...
func userConfigDir() string {
	return maddogHomeDir()
}

func maddogHomeDir() string {
...
func legacyXDGConfigPaths() []string {
	...
	if dir := cleanEnvDir("XDG_CONFIG_HOME"); dir != "" {
		add(filepath.Join(dir, "maddog", "config.toml"))
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
		add(filepath.Join(home, ".config", "maddog", "config.toml"))
	}
	return paths
}

func userSupportDir() string {
	if dir := cleanEnvDir("MADDOG_STATE_HOME"); dir != "" {
		return dir
	}
	return maddogHomeDir()
}
...
func legacyOSSupportDir() string {
	...
	path := filepath.Join(dir, "maddog")
	if current := maddogHomeDir(); current != "" && samePath(path, current) {
		return ""
	}
	return path
}
...
func userCacheDir() string {
	if dir := cleanEnvDir("MADDOG_CACHE_HOME"); dir != "" {
		return dir
	}
...
func userConfigDisplayPath() string {
	p := userConfigPath()
	if p == "" {
		return "<os-config-dir>/maddog/config.toml"
	}
	if home, err := osUserHomeDir(); err == nil && home != "" {
...
// from the current v1.8.1 Maddog-home config path.
...
// home. It is the single source for provider credentials saved by Maddog, so
// stale shell, Windows, project, or home env vars cannot silently override keys
// the user saved through setup or settings. "" when Maddog home can't be
// resolved.
...
// Used by `maddog --continue` / `--resume` to find the recent ones. Empty
...
// MemoryUserDir returns the maddog user state root (…/maddog), under which
...
var ConventionDirs = []string{".maddog", ".agents", ".agent", ".claude"}
...
// ~/.maddog/commands), the Maddog home commands dir, the legacy OS
// app-support dir if different, then the project's
// convention dirs (.claude/commands … .maddog/commands). Scanning the .claude /
...
func SourcePathForRoot(root string) string {
	root = resolveRoot(root)
	projectTOML := "maddog.toml"
	if root != "." {
		projectTOML = filepath.Join(root, "maddog.toml")
	}
