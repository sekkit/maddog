...
func TestExpandVars(t *testing.T) {
	t.Setenv("MADDOG_TEST_TOKEN", "sk-123")
	t.Setenv("MADDOG_TEST_EMPTY", "")

	cases := []struct{ in, want string }{
		{"Bearer ${MADDOG_TEST_TOKEN}", "Bearer sk-123"},
		{"${MADDOG_TEST_MISSING}", ""},                                   // unset, no default → empty
		{"${MADDOG_TEST_MISSING:-fallback}", "fallback"},                 // unset → default
		{"${MADDOG_TEST_EMPTY:-fallback}", "fallback"},                   // set-but-empty → default
		{"${MADDOG_TEST_TOKEN:-fallback}", "sk-123"},                     // set → value, default ignored
		{"no vars here", "no vars here"},                                   // untouched
		{"a${MADDOG_TEST_TOKEN}b${MADDOG_TEST_MISSING}c", "ask-123bc"}, // multiple refs
	}
	for _, c := range cases {
...
func TestExpandedPlugin(t *testing.T) {
	...
	e := PluginEntry{
		Name:    "x",
		Type:    "http",
		URL:     "https://api/${MADDOG_TEST_MISSING:-v1}",
		Args:    []string{"--token", "${MADDOG_TEST_KEY}"},
		Env:     map[string]string{"K": "${MADDOG_TEST_KEY}"},
		Headers: map[string]string{"Authorization": "Bearer ${MADDOG_TEST_KEY}"},
	}
	...
	if e.Headers["Authorization"] != "Bearer ${MADDOG_TEST_KEY}" {
...
func TestForbidReadRootsForRootResolvesRelativePathsAndScopedEnv(t *testing.T) {
	root := t.TempDir()
	cfg := Default()
	cfg.setExpansionEnv(map[string]string{"MADDOG_TEST_SECRET_DIR": "from-dotenv"})
	cfg.Sandbox.ForbidRead = []string{
		"relative-secret",
		"${MADDOG_TEST_SECRET_DIR}",
