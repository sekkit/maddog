...
// the v0.x ~/.maddog/config.json's mcpServers. Provider api_key_env values
// resolve from Maddog's global .env, not from project .env files.
...
// each project's maddog.toml + .mcp.json are resolved independently without
// changing the process cwd, while provider keys stay rooted in Maddog home.
func LoadForRoot(root string) (*Config, error) {
	...
	// project config's other fields, but do not let ./maddog.toml override
	...
	// Lowest priority: the v0.x ~/.maddog/config.json's mcpServers, so upgrading
...
// of resetting to defaults. Maddog's global .env is loaded so api_key_env
...
// MiMo providers needed by legacy refs that live outside maddog.toml, such as
