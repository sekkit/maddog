...
func TestTrustPluginReadOnlyToolInSourceForRootUpdatesMCPJSON(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(root, "maddog.toml")); !os.IsNotExist(err) {
...
name = "dida"
type = "http"
url = "https://maddog.example/mcp?access_token=toml"
[plugins.headers]
...
`), 0o644); err != nil {
  ...
  "dida": {
	...
	if updated.URL != "https://maddog.example/mcp" {
		t.Fatalf("updated URL = %q", updated.URL)
	}
