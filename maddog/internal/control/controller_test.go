...
func TestTwoModelShortChoiceReplySkipsPlanner(t *testing.T) {
	...
	if strings.Contains(reqText, "Maddog executor handoff") {
...
func TestRemoveMCPServerRemovesUnconnectedLazyPlaceholder(t *testing.T) {
	...
	t.Chdir(dir)
	if err := os.WriteFile("maddog.toml", []byte(`
[[plugins]]
name = "mock"
command = "mock-mcp"
tier = "lazy"
...
func TestApprovalPersistentBashPrefixRememberRule(t *testing.T) {
	...
	c := New(Options{
		...
		OnRemember: func(rule string) RememberResult {
			remembered = rule
			return RememberResult{Rule: rule, Path: "maddog.toml", Saved: true}
		},
	})
	go func() {
	...
	if len(notices) != 1 || !strings.Contains(notices[0], "Bash(go test:*)") || !strings.Contains(notices[0], "maddog.toml") {
...
func TestReloadCommandsFromFilesystem(t *testing.T) {
	...
	wsRoot := t.TempDir()
	cmdDir := filepath.Join(wsRoot, ".maddog", "commands")
...
// (higher priority) wins. ConventionDirs = [".maddog", ".agents", ".agent",
// ".claude"], scanned in reverse, so .maddog is highest priority.
func TestReloadCommandsSameNameAcrossDirs(t *testing.T) {
	...
	// Higher priority: .maddog/commands
	maddogDir := filepath.Join(wsRoot, ".maddog", "commands")
	writeCmdFile(t, maddogDir, "greet", "Maddog greet", "Hello from Maddog: $1")
	...
	// The winning version should be from .maddog (highest priority)
	sent, ok := c.CustomCommand("/greet world")
	if !ok {
		t.Fatal("/greet should be found")
	}
	if !strings.Contains(sent, "Hello from Maddog") {
		t.Errorf("expected .maddog version to win, got render: %q", sent)
	}
}
