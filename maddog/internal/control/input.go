...
- Say briefly in the first visible reply that the goal is being handled with AutoResearch and that state will live under .maddog/autoresearch/<task-id>/.
- Keep dynamic state out of MADDOG.md, AGENTS.md, project memory, system prompts, and tool schemas. Use project-local .maddog/autoresearch/ state only.
- For a new task, create a collision-resistant task id YYYYMMDD-HHMMSS-slug, check .maddog/autoresearch/ first, and append -2, -3, etc. only on collision. Reuse an explicitly supplied .maddog/autoresearch/<task-id>/ path exactly.
...
- Complete only after auditing every success criterion in task_spec.md against direct evidence. Public publishing, destructive changes, credential use, payments, external notifications, privacy-sensitive output, and cache-sensitive changes still require the normal Maddog gates.`
...
func shouldAutoStartResearchGoal(input string) bool {
	...
	if strings.Contains(lower, ".maddog/autoresearch/") {
		return true
	}
