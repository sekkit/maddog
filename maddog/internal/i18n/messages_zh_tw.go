...
var ChineseTraditional = Messages{
	...
	SetKeyHint:      "設定好 API key 後執行 `maddog chat`。",
	ConfigLabel:     "設定",
	ModelsLabel:     "模型",
	...
	Ready:           "已就緒",
	GetStarted:      "開始使用",
	StepScaffold:    "生成 maddog.toml",
	StepSetKey:      "設定 API key",

	InitHint:       "專案記憶（AGENTS.md）在會話內由模型生成：執行 `maddog chat`，然後 `/init` —— 模型會分析程式碼庫並寫入。設定請用 `maddog setup`。",
	StepSetKeyHint: "執行 `maddog setup`，或 export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "互動式會話",
	StepRunDesc:    "執行單次任務",
	HelpFooter:     "maddog help · 檢視全部命令",

	ChatTip:           "對話上下文將跨輪保留。輸入 'exit' 或按 Ctrl-D 退出。",
	TurnCancelled:     "已取消 — 回到提示符",
	NoSessionToResume: "沒有可恢復的會話 — 用 `maddog chat` 開一個新的",
	...
	OutputStyleHint:    "在 maddog.toml 設定 agent.output_style 即可啟用（下次會話生效）",
	ThemeHeader:        "主題：",
	...
	SlashMCPNone:                 "沒有設定 MCP 伺服器 — 在 maddog.toml 加一個 [[plugins]] 條目",
	...
	CmdReloadCmd:        "重新載入自訂命令",
	CmdDiffFold:         "切換 diff 摺疊/展開",
	CmdSandbox:          "檢視沙箱狀態",
	CmdEffort:           "設定推理強度",
	CmdAutoPlan:         "設定自動計畫模式",
	CmdReasonLang:       "設定可見思考語言",
	CmdMemoryV5:         "設定 Memory v5",
	CmdHelp:             "檢視命令列表",
	CmdTodo:             "清除任務清單",
	CmdQuit:             "退出會話",
	CmdCopy:             "複製一條助手回覆",
	CmdExport:           "匯出會話 Markdown",
	SlashCopyDone:       "已複製回覆到剪貼簿",
	SlashCopyEmpty:      "沒有可複製的助手回覆",
	SlashCopyListHeader: "要複製哪條助手回覆？",
	SlashExportDoneFmt:  "已匯出會話到 %s",
	...
	ArgEffortMax:        "最高推理",
	ArgThemeCurrent:     "當前",
	ArgLanguageAuto:     "從 MADDOG_LANG / 系統 locale 自動偵測",
	...
	ListHooksNone:       "無生效 hooks — 在 .maddog/settings.json（專案，需信任後）或 ~/.maddog/settings.json（全域）設定",
	ListMcpHeader:       "MCP 伺服器",
	ListMcpNone:         "未連線 MCP 伺服器 — 在 maddog.toml（[[plugins]]）或專案 .mcp.json 中新增",

	MemoryNone:             "還沒有載入任何記憶 — 輸入 “/remember 內容” 可快速記錄，也可以在專案根目錄建立 MADDOG.md",
	...
	MissingKeyIntro:       "maddog.toml 已設定好 — 只差一個 API key 就可以開始。",
	WroteFileFmt:          "已寫入 %s",
	SetupComplete:         "設定完成。",
	SetupCancelled:        "設定已取消。",
	TryHintFmt:            "試試: %s",
	NextHint:              "下一步：設定 API key（執行 `maddog setup` 或 export DEEPSEEK_API_KEY=...），然後執行 `maddog run \"你的任務\"`。",
	...
	SkipStaleCustomEntryFmt:    "跳過 maddog.toml 裡的舊 %q 條目（指向 %s）— 請手動從 [[providers]] 裡刪除",
	...
	UsageRunHint:              "用法：maddog run [--model NAME] <task>",
	ErrorPrefix:               "錯誤：",
	ReconfigureOnUnknownModel: "設定的模型已不可用 —— 重新執行引導設定。",
	WriteConfigErr:            "寫入設定失敗：",
	WriteEnvErr:               "寫入 credentials 失敗：",

	ProviderErrBadRequest:          "請求格式錯誤 (HTTP 400)：請求體被拒絕，通常是程式缺陷。若持續出現請回報。",
	ProviderErrAuth:                "認證失敗 (HTTP 401)：API key 缺失、錯誤或已過期。請檢查 Maddog credentials 或專案 .env 中的金鑰，或執行 `maddog setup`。",
	...
	UsageBody: `maddog — 由設定和插件驅動的 coding agent（多模型）
...
範例：
  ...
  maddog run "把 main.go 裡的 TODO 實現掉"
  maddog run --model mimo-pro "給這個函式補單元測試"
  echo "解釋這段程式碼" | maddog run

設定：
  優先順序：flag > ./maddog.toml > ~/.config/maddog/config.toml > 內建預設值
  金鑰透過 api_key_env 從環境變數注入（如 DEEPSEEK_API_KEY）。
  執行 'maddog setup' 生成設定；詳見 docs/SPEC.md。
`,
	...
	GoalSetFmt:                 "目標已設定 → %s",
	GoalCleared:                "目標已清除",
	ProviderErrAuthRejected:    "認證失敗 (HTTP 401)：服務端拒絕了你的 API key。可能是 key 錯誤或已過期，也可能是服務端出現瞬時鑑權/額度問題——已退避重試仍失敗。請稍後再試，或檢查 .env 中的金鑰 / 執行 `maddog setup`。",
