...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — maddog setup
	StepSetKey      string // step 2 command label

	// `maddog init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // maddog chat step desc
	StepRunDesc    string // maddog run step desc
	HelpFooter     string // dim footer linking to maddog help
	...
	WroteFileFmt          string // "Wrote %s" — used for maddog.toml and credentials
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from maddog.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: maddog run [--model NAME] <task>"
	...
	WriteEnvErr               string // "write credentials:" — prefix for env-write failure
...
//
// Priority: override > MADDOG_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"MADDOG_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
