...
func isolateMigrationHome(t *testing.T) string {
	...
	t.Setenv("MADDOG_HOME", filepath.Join(home, "new-maddog"))
...
func TestRunLegacyRescueImportsSessionsAndEmitsProgress(t *testing.T) {
	...
	legacyDir := filepath.Join(home, ".maddog", "sessions")
...
func TestRunLegacyRescueImportsMemory(t *testing.T) {
	home := isolateMigrationHome(t)
	legacyRoot := filepath.Join(home, ".maddog")
	if err := os.MkdirAll(filepath.Join(legacyRoot, "memory", "global"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(legacyRoot, "MADDOG.md"), []byte("legacy user memory\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, path := range []string{
		filepath.Join(config.MemoryUserDir(), "MADDOG.md"),
...
func TestRunLegacyRescueCommandImportsFromExplicitInstallDir(t *testing.T) {
	home := isolateMigrationHome(t)
	installRoot := filepath.Join(home, "Custom Maddog")
...
func TestMigrateLegacySessionSourcesSkipsCurrentProjectTree(t *testing.T) {
	...
	if !samePath(config.MemoryUserDir(), filepath.Join(home, ".maddog")) {
		t.Skip("current Maddog home is not ~/.maddog on this platform")
	}
