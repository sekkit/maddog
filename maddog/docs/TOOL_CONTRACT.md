...
This document records the provider-visible contract for Maddog compile-time built-in tools. It is generated from the same canonical schema path used by the runtime registry.
...
| `goal_control` | false | Inspect and explicitly manage the current autonomous goal. Use `get` to read durable state; use `create` only for an explicitly requested objective when no goal is running; use `update complete` only after host readiness checks pass; use `update blocked` only after the same blocker is reported on three consecutive goal turns; and use `clear` only when the user explicitly asks to stop the goal. |
