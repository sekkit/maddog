...
优先级：**flag > `./maddog.toml` > 用户配置文件 > 内置默认值**。用户配置位于操作系统配置目录：
Linux 为 `~/.config/maddog/`，macOS 为 `~/Library/Application Support/maddog/`，Windows 为 `%AppData%\maddog\`。
...
# language    = "zh"               # 界面语言；为空则按 $LANG / $MADDOG_LANG 自动检测
...
# password_hash = ""           # 用 maddog serve --hash-password --password '...' 生成
...
`[agent].plan_mode_allowed_tools` 用于把 Maddog 无法自动分类的自定义/外部工具声明为额外只读工具。
...
多数日常设置应写在 `config.toml` 或前文提到的 Maddog 全局 `.env` 中。下面这些变量是进程级高级开关；
需要在启动 Maddog 之前设置。项目 `.env` 不是 Maddog 控制变量的运行时来源。

`MADDOG_MEMORY_COMPILER_LLM_CLASSIFICATION=true` 会为 Memory v5 启用可选的 LLM 任务/聊天分类器。
默认关闭，此时 Maddog 使用本地 heuristic classifier，不会产生额外 provider 调用。开启后，分类缓存未命中时，
Maddog 可能先通过已配置 provider 发送一个很小的分类请求，再决定用户输入是任务还是普通对话；这会增加少量延迟、
...
`maddog serve` 会用同一个本地 Maddog 引擎启动浏览器 UI。适合不安装桌面端但想用可视化界面、
...
桌面端快捷键在 **设置 → 快捷键** 中管理。选择一行后按下新的组合键，Maddog 会为桌面端保存该绑定。
...
| 终端原生选择 | 复制 transcript 文本 | Maddog 默认不启用鼠标报告，因此终端自己的选择/复制仍可使用。 |
...
`maddog chat` 会在 writer 调用前征求同意（普通工具为 `1` 本次 · `2` 本会话允许此范围 · `3` 总是允许此范围（保存） · `4` 拒绝；Bash 可额外选择命令前缀授权）；
其中 Bash 默认按具体命令记，也可按安全推导出的命令前缀记（如 `Bash(go test:*)`）；文件编辑类工具的本会话授权按编辑能力记，持久授权则写入 `Edit(<path>)` 文件路径规则；
`maddog run` 保持自主运行但仍然遵守 `deny`。
...
`mcpServers.<server>.trusted_read_only_tools`，普通 Maddog plugin 会写入用户级 Maddog config。
...
各自暴露的内容。`make build` 还会产出 `bin/maddog-plugin-example`——一个可直接运行的
...
**从旧安装升级或迁移？** Maddog 不会自动读取或导入 legacy Maddog 配置，例如
`~/.maddog`、`~/.config/maddog`、`.maddog` 或 `maddog.toml`。这样 Maddog 与已安装的
DeepSeek Maddog 不会互相改动配置或运行时状态。需要保留的 MCP 服务器请手动复制到
`maddog.toml` 的 `[[plugins]]` 或 `.mcp.json`。

## 斜杠命令

`maddog chat` 里，内置命令（`/compact`、`/new`、`/clear`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/mcp`、`/skills`、`/hooks`、`/memory`、`/output-style`、`/sandbox`、`/language`、`/auto-plan`、`/reasoning-language`、`/help`）在本地执行——`/help` 可列出全部。
...
是放在 `.maddog/commands/`（项目）或 `~/.config/maddog/commands/`（用户）下的 Markdown 文件——
`review.md` 即 `/review`，子目录构成命名空间（`git/commit.md` → `/git:commit`）。文件正文
是 prompt 模板，调用即作为一轮对话发出。

`/memory` 会同时列出记忆文档（`MADDOG.md` / `AGENTS.md`）和已保存的 auto-memory 条目。
...
Memory v5 在 CLI/TUI、`maddog serve` 和桌面端默认开启，因为这些入口共用同一套本地
controller。它会把本地、按项目隔离的执行轨迹和编译器状态写在 Maddog home 下，并且只有
...
`maddog config memory-v5 off|on|status`。桌面端还可以在设置 → 通用 → Memory v5 中控制。
...
CLI/TUI 和 `maddog serve` 使用同一个 user/global 配置。项目内的 `maddog.toml` 不能覆盖
这个 user/global 设置。CLI 命令会更新底层配置；高级用户也可以手动编辑 Maddog home 下的
用户配置：
...
`maddog run --metrics <path>` 时，JSON 还会输出内容无关的 `memory_compiler_*` 汇总字段，
...
Goal 是长期目标的统一运行机制。普通 `/goal` 继续走轻量 Goal：Maddog 会持续推进，直到
...
`.maddog/autoresearch/<task-id>/` 任务目录。高级用户可以用
...
它会创建或复用项目级 `.maddog/autoresearch/<task-id>/` 目录。新任务默认使用
...
`maddog setup` 刻意保持首次体验极简：选 provider → 输入 key（所选 provider 的所有
SKU 都会启用）。若要让两个模型协同（执行器 + 规划器，各自独立、缓存稳定的
session），向导后手动在 `maddog.toml` 加一行即可：
...
Planner 会看到已加载的 `MADDOG.md` / `AGENTS.md` 记忆，并拿到一小组只读研究工具，
...
再写进项目的 `./maddog.toml`，例如超大代码库需要更高的 planner 上限。
...
的任务会自动进入 plan mode：Maddog 先只读生成计划，待用户批准后才
编辑文件或执行有副作用的命令。`auto_plan_classifier` 可以指定便宜的 provider，例如
`deepseek-flash`；它只在边界输入上调用，分类失败会回退到启发式规则。也可以用
`maddog chat` 里的 `/auto-plan off|on` 修改用户级设置，或在 shell/脚本里用
`maddog config auto-plan off|on`。可见思考语言也采用同样形态：chat 里用
`/reasoning-language auto|zh|en`，shell/脚本里用
`maddog config reasoning-language auto|zh|en`。只有明确想写项目级覆盖时，才给
shell 命令加 `--local`。
