...
Resolution order: **flag > `./maddog.toml` > the user config file >
built-in defaults**. The user config lives in your OS config dir: `~/.config/maddog/`
on Linux, `~/Library/Application Support/maddog/` on macOS, `%AppData%\maddog\` on
...
# password_hash = ""           # bcrypt hash generated with maddog serve --hash-password --password '...'
...
external tools Maddog cannot classify itself. For MCP/plugin tools, a concrete
...
Most day-to-day settings belong in `config.toml` or the global Maddog `.env`
described above. The variables below are process-level advanced switches; set
them before launching Maddog. Project `.env` files are not a runtime source for
Maddog control variables.

`MADDOG_MEMORY_COMPILER_LLM_CLASSIFICATION=true` enables the optional LLM
task/chat classifier for Memory v5. By default it is disabled, and Maddog uses
...
```bash
MADDOG_MEMORY_COMPILER_LLM_CLASSIFICATION=true maddog
```

For development runs, prefix the command that starts the process, for example:

```bash
MADDOG_MEMORY_COMPILER_LLM_CLASSIFICATION=true wails dev -forcebuild
```
...
`maddog serve` starts the same local engine behind a browser UI. Use it when
you want a desktop-style surface without installing the desktop app, when running
Maddog on a remote development box through a tunnel, or when you want a
...
```bash
maddog serve --auth token
maddog serve --addr 0.0.0.0:8787 --auth token
maddog serve --auth password --password 'temporary-password'
```
...
```bash
maddog serve --hash-password --password 'strong-password'
...
new key combination, and Maddog saves it for the desktop app. Conflicting
...
| Terminal native selection | Copies transcript text | Maddog does not enable mouse reporting by default, so terminal selection/copy remains available. |
...
`maddog chat` can grant Bash as an exact command or as a conservative command
...
`maddog run` stays autonomous but still honours `deny`.
...
`mcpServers.<server>.trusted_read_only_tools`, while ordinary Maddog plugins
are updated in the user's Maddog config. Trust only side-effect-free readers;
...
**Upgrading or moving from an older install?** Maddog intentionally does not
auto-read or auto-import legacy Maddog configuration such as `~/.maddog`,
`~/.config/maddog`, `.maddog`, or `maddog.toml`. This keeps Maddog and an
installed DeepSeek Maddog from changing each other's configuration or runtime
state. Copy any MCP servers you still want into `maddog.toml`'s `[[plugins]]` or
a `.mcp.json` by hand.

## Slash commands

In `maddog chat`, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
another branch. **Custom commands** are Markdown files under `.maddog/commands/`
(project) or `~/.config/maddog/commands/` (user) — `review.md` becomes
...
`/memory` lists both memory documents (`MADDOG.md` / `AGENTS.md`) and saved
...
Memory v5 is enabled by default across the CLI/TUI, `maddog serve`, and the
desktop app because they all share the same local controller. It records local,
project-scoped execution traces and compiler state under Maddog home, then
...
session, or with `maddog config memory-v5 off|on|status` from a shell/script.
...
CLI/TUI and `maddog serve` use the same user/global config. Project
`maddog.toml` files cannot override this user/global setting. The CLI command
...
aggregate metrics upload pipeline. When `maddog run --metrics <path>` is used,
...
objectives stay lightweight: Maddog keeps working until the goal is complete,
...
`.maddog/autoresearch/<task-id>/` directory. Advanced users can force it with
...
`.maddog/autoresearch/<task-id>/` directory. For new tasks, the default id
shape is `YYYYMMDD-HHMMSS-slug`, such as `20260618-224530-cache-audit`; Maddog
...
broad requirement. Dynamic run state stays in `.maddog/autoresearch/...`, not
in `MADDOG.md`, `AGENTS.md`, project memory, tool schemas, or the cache-stable
...
`./maddog.toml` only when that repository needs a shared override, such as a
...
`maddog config auto-plan off|on` from a shell/script. The visible reasoning
language uses the same shape: `/reasoning-language auto|zh|en` in chat, or
`maddog config reasoning-language auto|zh|en` in a shell/script. Pass
