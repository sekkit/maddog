...
1. 打开桌面端“设置 -> Hooks”。
2. 选择范围：
   - “全局”：保存到 `~/.maddog/settings.json`，始终加载。
   - “项目”：保存到当前工作区的 `.maddog/settings.json`，必须点击“信任此工作区”后才会加载。
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "match": "bash",
        "command": "node .maddog/hooks/check-bash.js",
...
| 范围 | 文件 | 是否需要信任 | 加载顺序 |
| --- | --- | --- | --- |
| 全局 | `~/.maddog/settings.json` | 不需要 | 项目 hooks 之后 |
| 项目 | `<workspace>/.maddog/settings.json` | 需要 | 全局 hooks 之前 |

项目 hooks 的信任状态不写在项目文件里，而是写在用户自己的 `~/.maddog/trust.json`。这样克隆来的仓库不能靠提交 `.maddog/settings.json` 自动执行命令。
...
```json
{
  "hooks": {
    "PreToolUse": [
      { "match": "bash", "command": "node .maddog/hooks/pre-tool.js" }
    ],
    "UserPromptSubmit": [
...
```json
[
  { "event": "PreToolUse", "match": "bash", "command": "node .maddog/hooks/pre-tool.js" },
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "match": "bash",
        "command": "node .maddog/hooks/block-dangerous-bash.js",
...
`.maddog/hooks/block-dangerous-bash.js`：
