...
Starting with **Maddog v1.8.1**, Maddog uses one user-facing home directory
for global configuration and user-owned state. CLI and desktop share this
location.

## Maddog Home

| Platform | Maddog home |
| --- | --- |
| macOS | `~/.maddog` |
| Linux | `~/.maddog` |
| Windows | `%APPDATA%\maddog` |

Set `MADDOG_HOME` to override Maddog home for tests, CI, or portable
...
| Data | Path |
| --- | --- |
| Global config | `<Maddog home>/config.toml` |
| Global provider credentials | `<Maddog home>/.env` |
| Legacy credentials import source | `<Maddog home>/credentials` |
| Global slash commands | `<Maddog home>/commands/` |
| Global skills | `<Maddog home>/skills/` |
| Global hooks | `<Maddog home>/settings.json` |
| Hook trust store | `<Maddog home>/trust.json` |
| Sessions | `<Maddog home>/sessions/` |
| Archives | `<Maddog home>/archive/` |
| Memory | `<Maddog home>/memory/` and `<Maddog home>/projects/` |

The global user config is named `config.toml`. Project-local config files keep
the name `maddog.toml`. If someone says "global maddog.toml", they usually
mean `<Maddog home>/config.toml`.

## Global `config.toml`

`<Maddog home>/config.toml` stores non-secret configuration shared by the CLI
and desktop app. It may contain the same provider, plugin, UI, desktop, tool,
skill, sandbox, bot, and agent settings that Maddog renders into user config.
...
When a custom provider is added from the desktop settings or `maddog setup`,
Maddog stores a generated `api_key_env` in `config.toml` and writes the secret
...
Maddog derives the default from the provider name. Names that normalize to
...
`<Maddog home>/.env` is the single runtime source for provider API keys saved
by Maddog. The setup wizard, desktop settings, CLI missing-key prompts, and
...
- multiline values are rejected by Maddog writes;
- keys must use shell-style names such as `DEEPSEEK_API_KEY`;
- `# maddog-cleared KEY` comments are non-secret tombstones written after a key
  is deleted so legacy stores do not silently re-import it;
- Maddog writes this file with restricted permissions where the OS supports
  them.

For provider requests, Maddog resolves only this global `.env`. Project `.env`
...
into the process environment, and Maddog control variables such as
`MADDOG_HOME`, `MADDOG_STATE_HOME`, and `XDG_CONFIG_HOME` are ignored there.

Caches remain in the OS cache directory, for example
`~/Library/Caches/maddog` on macOS, `$XDG_CACHE_HOME/maddog` or
`~/.cache/maddog` on Linux, and `%LOCALAPPDATA%\maddog\cache` on Windows.
Set `MADDOG_CACHE_HOME` to override the cache root.
...
command-line flags
> project ./maddog.toml
> global <Maddog home>/config.toml
...
```text
macOS/Linux: ~/.maddog/config.toml
Windows:     %APPDATA%\maddog\config.toml
```

## Legacy Migration

Starting with **v1.8.1**, Maddog automatically checks legacy locations on
startup before the first config load. Migration is synchronous, one-time, and
non-destructive: old files are copied or converted to Maddog home and left
untouched.

Legacy config sources include:

```text
~/Library/Application Support/maddog/config.toml
~/.config/maddog/config.toml
~/.maddog/maddog.toml
~/.maddog/config.json
```

Legacy credentials, memory files, and sessions are also imported into Maddog
home when the new destination does not already exist. Legacy provider keys are
copied into `<Maddog home>/.env` only when that file does not already contain
...
Starting in **v1.9.1**, Maddog also backfills MCP servers from known legacy
paths, legacy `config.json`, desktop-registered projects, and restored tab
projects into the global `<Maddog home>/config.toml`. Existing global
...
If Maddog has already created the new home directory but some legacy data was
...
```text
/migrate --from "D:\OldMaddog"
```

The explicit form imports sessions only. The path may be the old install
directory, a `.maddog`/data directory, or the `sessions` directory itself;
Maddog checks the common layouts below that root and uses a source-specific
...
existing `<Maddog home>/config.toml`; if the new config already exists, copy
...
- `/migrate` is available only in Go-based Maddog builds that include the
  command. If Maddog reports `unknown command`, upgrade first and rerun it.
