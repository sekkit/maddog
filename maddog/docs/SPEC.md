...
- **Collaboration mode is separate from tool approval.** The desktop composer
  ...
  `.maddog/autoresearch/YYYYMMDD-HHMMSS-slug/` and keeps dynamic run state out
  of `MADDOG.md`, `AGENTS.md`, project memory, tool schemas, and the
...
`[serve]` controls the HTTP browser frontend used by `maddog serve`. The
...
`multi_edit`, `move_file`) to `workspace_root` (default cwd), the Maddog user
