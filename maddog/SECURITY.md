...
Maddog security fixes are prioritized for the currently developed Go rewrite
...
- Affected Maddog version, commit, operating system, and installation method.
...
Maddog is a local coding agent. Many features intentionally operate on the
...
Maddog supports `@path` references so users can include local files and images
...
- A local user intentionally asks Maddog to read, edit, or send their own
