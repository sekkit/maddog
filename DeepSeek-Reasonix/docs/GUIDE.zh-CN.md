...
**从旧安装升级或迁移？** Maddog 不会自动读取或导入 legacy Reasonix 配置，例如
`~/.reasonix`、`~/.config/reasonix`、`.reasonix` 或 `reasonix.toml`。这样 Maddog 与已安装的
DeepSeek Reasonix 不会互相改动配置或运行时状态。需要保留的 MCP 服务器请手动复制到
