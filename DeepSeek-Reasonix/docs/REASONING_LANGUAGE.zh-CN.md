...
桌面端设置会写入用户级默认值。项目仍然可以通过 `./maddog.toml` 覆盖。
...
斜杠命令会写入用户级设置，并立即更新当前 chat controller，后续 turn 生效。它不会改写当前项目的 `maddog.toml`；如果要写项目级覆盖，请使用带 `--local` 的 shell 命令。
...
```text
./maddog.toml > 用户 config.toml > 内置默认值
```
