...
func TestRefinementStrategyIsDefaultOffAndGateControlled(t *testing.T) {
	...
	off := EvaluateRefinementStrategy(RefinementInput{
		Strategy:              strategy,
		BudgetRemainingTokens: 1000,
		KillSwitchEnabled:     true,
		HumanApproved:         true,
	})
	if off.CanStart || off.Status != "off" || off.HumanGate != nil {
		t.Fatalf("default-off refinement should not start or gate: %+v", off)
	}

	strategy.Enabled = true
	noKill := EvaluateRefinementStrategy(RefinementInput{Strategy: strategy, BudgetRemainingTokens: 1000, KillSwitchEnabled: false, HumanApproved: true})
