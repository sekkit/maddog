...
func TestEvidenceLedgerSnapshotCopiesReceipts(t *testing.T) {
	ledger := evidence.NewLedger()
	raw := json.RawMessage(`{"command":"go test"}`)
	ledger.Record(evidence.Receipt{ToolName: "bash", Args: raw, Success: true, Paths: []string{"a.go"}})
	snap := ledger.Snapshot()
	snap[0].Args[0] = '{'
	snap[0].Paths[0] = "mutated.go"
	again := ledger.Snapshot()
	if string(again[0].Args) != `{"command":"go test"}` || again[0].Paths[0] != "a.go" {
		t.Fatalf("snapshot should deep-copy mutable fields: %+v", again[0])
	}
}
