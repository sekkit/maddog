...
func cloneReceipts(in []evidence.Receipt) []evidence.Receipt {
	if len(in) == 0 {
		return nil
	}
	out := make([]evidence.Receipt, len(in))
	for i, r := range in {
		out[i] = r
		if r.Args != nil {
			out[i].Args = append(json.RawMessage(nil), r.Args...)
		}
		out[i].Paths = append([]string(nil), r.Paths...)
		out[i].Todos = append([]evidence.TodoItem(nil), r.Todos...)
		if r.TodoStep != nil {
			step := *r.TodoStep
			out[i].TodoStep = &step
		}
