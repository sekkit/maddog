...
// codegraphCommand backs `maddog codegraph` — managing the CodeGraph
// code-intelligence runtime that Maddog otherwise fetches lazily on first use.
...
func codegraphStatus() int {
	...
	if p, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-13s %s\n", "resolved:", "(not installed — run `maddog codegraph install`)")
	}
	return 0
}

func codegraphUsage() {
	fmt.Print(`maddog codegraph — manage the CodeGraph code-intelligence runtime

Usage:
  maddog codegraph install   download + cache the runtime for this platform
