...
// mcp.go holds the MCP server-management surface shared by the `maddog mcp`
...
// mcpCommand implements `maddog mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-16s (built-in, not installed)%s  run `maddog codegraph install`", "codegraph", codegraphMeta)
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: maddog mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to maddog.toml).

Usage:
  maddog mcp list
  maddog mcp add <name> <command> [args...]        stdio server
  maddog mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  maddog mcp add <name> --sse  <url>               remote (legacy SSE)
  maddog mcp import                                import Codex-enabled servers from cc-switch
  maddog mcp remove <name>
...
Examples:
  maddog mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  maddog mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
