...
func TestExpandPastedBlocksImage(t *testing.T) {
	m := &chatTUI{pastedBlocks: []pastedBlock{
		{label: "[image #1]", text: "@.maddog/attachments/clipboard-20260601-010203.000001.png", image: true},
		{label: "[Pasted text #2 · 3 lines]", text: "a\nb\nc"},
	}}
	got := m.expandPastedBlocks("look at [image #1] and [Pasted text #2 · 3 lines]")
	want := "look at @.maddog/attachments/clipboard-20260601-010203.000001.png and " +
...
func TestDisplayLineForImageRefs(t *testing.T) {
	got := displayLineForImageRefs("describe @.maddog/attachments/clipboard-20260601-010203.000001.png @docs/readme.md")
	want := "describe [image1] @docs/readme.md"
	if got != want {
		t.Fatalf("displayLineForImageRefs = %q, want %q", got, want)
	}
}

func TestDisplayLineForImageRefsIgnoresReasonixAttachmentRefs(t *testing.T) {
	got := displayLineForImageRefs("describe @.reasonix/attachments/clipboard-20260601-010203.000001.png")
	want := "describe @.reasonix/attachments/clipboard-20260601-010203.000001.png"
	if got != want {
		t.Fatalf("displayLineForImageRefs = %q, want %q", got, want)
	}
}
