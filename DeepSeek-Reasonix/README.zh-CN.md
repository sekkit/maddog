...
> [!IMPORTANT]
> **Maddog 1.0 是用 Go 从零重写的版本** —— 本分支(`main-v2`)已是新的默认分支,后续开发都在这里。
> 早期的 `0.x` TypeScript 版本转为 **legacy**,保留在 [`v1`](https://github.com/sekkit/maddog/tree/v1) 分支(仅维护)。
> 详见**[迁移指南](./docs/MIGRATING.md)**。`npm i -g maddog` 仍是安装命令——`1.0.0`+ 装的是 Go 二进制,`0.x` 是 legacy TS 版。
...
- **配置驱动**：provider、agent、启用的工具、插件全部在 `maddog.toml` 中声明，
  内核无硬编码模型。
...
```sh
npm i -g maddog                  # 任意系统;自动拉取对应平台的原生二进制
...
```sh
maddog setup                      # 配置向导 → ./maddog.toml
export DEEPSEEK_API_KEY=sk-...  # 或写入 .env（见 .env.example）
maddog chat                       # 然后在会话里运行 /init 生成 AGENTS.md（项目记忆）
maddog run "把 main.go 里的 TODO 实现掉"
maddog run --model mimo-pro "给这个函数补单元测试"
echo "解释这段代码" | maddog run
```
...
`mode`）。`maddog chat` 会在 writer 调用前征求同意（`1` 本次 · `2` 本会话 · `3` 拒绝，兼容 `y/a/n`）；
...
**从 `0.x` 升级？** Maddog 不再自动读取旧的 `~/.reasonix/config.json`，这样已安装的
DeepSeek Reasonix 与 Maddog 会保持隔离。仍想复用的 MCP 服务器，请手动复制到
`maddog.toml` 的 `[[plugins]]` 或项目 `.mcp.json`。

### 斜杠命令

`maddog chat` 里,内置命令(`/compact`、`/new`、`/rewind`、`/tree`、`/branch`、`/switch`、`/todo`、`/model`、`/effort`、`/mcp`、`/help`)在本地执行。
...
`maddog config auto-plan off|on`。只有明确想写项目级覆盖时，才给 shell 命令加
`--local`。
...
grep、web_fetch）、TOML 配置、交互式 `maddog setup` 向导、双模型协同（执行器 + 规划器，
...
（`.maddog/commands/*.md`）、`@file` / `@resource` 引用、外加可运行的参考插件
（`cmd/maddog-plugin-example`）、harness 主循环、CLI。chat 在终端普通缓冲区运行(原生
...
如果 Maddog 帮你省了时间或 token，欢迎请杯咖啡。捐助不会换来 feature 优先级，也不会影响 issue 的处理顺序——就是「谢谢」。
...
下面这些朋友的工作塑造了 Maddog 今天的样子 —— 综合 commit 数和代码量两个维度。
...
<p align="center">
  <a href="https://github.com/sekkit/maddog/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=sekkit/maddog&max=100&columns=12" alt="sekkit/maddog 贡献者" width="860"/>
  </a>
</p>
...
<p align="center">
  ...
  <br/>
  <sub>由 <a href="https://github.com/sekkit/maddog/graphs/contributors">sekkit/maddog</a> 社区共建</sub>
</p>
