...
extern void MaddogMarkSystemQuit(void);
...
static NSApplicationTerminateReply maddogApplicationShouldTerminate(id self, SEL _cmd, NSApplication *sender) {
    MaddogMarkSystemQuit();
...
static void maddogWailsContextQuit(id self, SEL _cmd) {
    MaddogMarkSystemQuit();
...
void installMaddogSystemQuitHook(void) {
    ...
    if (method != NULL) {
        IMP replacement = (IMP)maddogApplicationShouldTerminate;
    ...
    if (quitMethod != NULL) {
        IMP replacement = (IMP)maddogWailsContextQuit;
