...
- **Status**: complete
- **Execution note**: test-first
- **Files**: `Maddog/internal/review/rules.go`, `Maddog/internal/review/rules_test.go`, `Maddog/internal/review/report.go`, `Maddog/internal/review/report_test.go`, `Maddog/internal/cli/review.go`, `Maddog/internal/cli/review_test.go`, `Maddog/internal/skill/builtins.go`, `Maddog/internal/skill/skill_test.go`
- **Depends on**: Post-v1 Unit F1, Post-v1 Unit G1
- **Result**: Added `internal/review` deterministic rules for secret-like tokens, unsafe remote shell installers, destructive SQL, ignored errors, and large-diff risk markers, plus a prompt builder that asks the LLM to explain/prioritize grounded findings. The existing `maddog review` CLI now runs these rules before launching the review subagent, redacts secrets in both findings and diff text, fences untrusted evidence/context, and supplies changed-file context or explicit diff-only fallback. The built-in `review` skill now instructs subagents to use deterministic rules and code intelligence context when available.
- **Review**: Multi-agent review found blockers where rules were dead code from the actual review path and secret findings could leak raw tokens into the LLM prompt. Fixed by wiring `AnalyzeUnifiedDiff`/`BuildLLMPrompt` into `maddog review`, redacting secrets in raw diff and finding evidence regardless of length, fencing untrusted evidence, skipping unified-diff metadata lines, parsing plain unified `+++` paths, and suppressing shell/SQL example findings in docs/comments.
- **Verification**: `go test ./internal/review ./internal/cli ./internal/skill -run 'TestAnalyzeUnifiedDiff|TestBuildLLMPrompt|TestBuildReviewTask|TestBuiltinReviewMentionsHybridDeterministicRules|TestBuiltinSubagentSkillsDeclareAllowedTools' -count=1`; `go test ./... -count=1`.
- **Commit**: 817151089
