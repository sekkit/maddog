...
| Offline replay self-improvement | C2-A, C2-B, C2-C, C2-D, G1, G2, G3 | Replay bundle capture, runner, scorer, guardrail, promotion, candidate store, desktop skill candidate UI | [microsoft/SkillOpt](https://github.com/microsoft/SkillOpt) and paper [arXiv:2605.23904](https://arxiv.org/abs/2605.23904); [iamzhihuix/skills-manage](https://github.com/iamzhihuix/skills-manage); [PabloNAX/ultracode-skill](https://github.com/PabloNAX/ultracode-skill); [pat-jj/harness-1](https://github.com/pat-jj/harness-1) | Capture or load a replay bundle, run `skilleval` provider evaluation for promotion-grade evidence, verify dry-run remains preview-only, guardrail outcome, candidate status, desktop promotion/rollback audit |
...
| Offline replay self-improvement | Promotion-grade CLI/GUI evaluation must use provider replay over at least 5 distinct held-out bundles by default, reject duplicate bundle IDs/paths, reject the candidate source bundle, require every score to be >= 0.70, reject success-rate regressions, and persist aggregate score/guardrail plus promotion-grade provenance when requested. Dry-run remains preview-only and cannot promote. | `go test ./internal/skilleval ./internal/cli -run 'SkillEval|Guardrail|EvaluateCandidate|Promote' -count=1`; JSON `skilleval` output with `bundles`, `bundle_ids`, `scores`, and `persisted`; desktop candidate provenance fields. |
| Offline replay GUI | The GUI must expose candidate evaluation request/provenance, promotion, rollback, rejection, status, score, guardrail reason, source bundle, provider/model, held-out bundle count, and audit metadata from Settings/Skills or Capabilities without weakening the default 5-bundle guardrail. | `go test . -run 'SkillCandidate|EvaluateSkillCandidate' -count=1` in `desktop`; `capabilities-skill-candidates.test.ts`. |
...
| Code intelligence benchmark | `cmd/codeintelbench` and `cmd/e2ebench` must build and pass package tests; CLI mock benchmark is opt-in only; desktop benchmark must route to the selected real built-in/sidecar adapter or report an explicit degraded/external-contract state instead of local smoke. | `go test ./cmd/codeintelbench ./cmd/e2ebench -count=1`; `go test . -run 'RunCodeIntelligenceBenchmark' -count=1` in `desktop`; configured HyperGraphRAG sidecar health/index/query smoke when available. |
...
| Code intelligence benchmark commands | Partial real-mode ready | `cmd/codeintelbench` and `cmd/e2ebench` are present; mock report path is explicit via `-include-mock`; selected desktop benchmarks require a real CodeGraph launcher or configured HyperGraphRAG sidecar for ready evidence. |
...
| 2026-07-03 | Real-gap focused verification: `go test ./internal/skilleval ./internal/control ./internal/cli -run "CaptureBundle|Guardrail|RunCapturesOfflineReplayBundle|SkillEvalCommand|HyperGraphRAG" -count=1`; `go test ./cmd/codeintelbench ./cmd/e2ebench ./internal/codegraph -run "CodeIntel|Benchmark|Regression|HyperGraphRAG" -count=1`; in `desktop`, `go test . -run "CodeIntel|Benchmark|RunCodeIntelligenceBenchmark|SkillCandidate|EvaluateSkillCandidate|Regression" -count=1`; frontend `npm run test:all`; frontend `npm run build` | Pass: GUI skilleval now records promotion-grade provider replay provenance, dry-run is preview-only, strict real gate coverage passes, and frontend build warnings remain non-fatal. |
